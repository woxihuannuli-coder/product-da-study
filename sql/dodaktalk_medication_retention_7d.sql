...
SELECT
  ...
  ROUND(retained.retained_users / total.total_users * 100, 2) AS medication_retention_rate
FROM (
  SELECT
...
CROSS JOIN (
  ...
  FROM (
    SELECT
      R.user_id,
      R.signup_date,
      COUNT(DISTINCT DATE(E.event_time, 'Asia/Seoul')) AS dose_checked_days
    FROM (
      SELECT
        user_id,
        signup_date
    ...
    GROUP BY
      R.user_id,
      R.signup_date
    HAVING COUNT(DISTINCT DATE(E.event_time, 'Asia/Seoul')) >= 3
  )
) AS retained;